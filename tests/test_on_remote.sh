...
#
# Syncs the repo, runs baseline and FlashHead benchmarks in separate
# Docker containers (for clean GPU memory between runs), then compares
...
#
#   BASELINE_MODEL=embedl/Cosmos-Reason2-2B-W4A16-Edge2 \
#   FLASHHEAD_MODEL=embedl/Cosmos-Reason2-2B-W4A16-Edge2-FlashHead-clean \
#   HF_TOKEN=hf_xxx \
...
BASELINE_MODEL="${BASELINE_MODEL:-embedl/Cosmos-Reason2-2B-W4A16-Edge2}"
FLASHHEAD_MODEL="${FLASHHEAD_MODEL:-embedl/Cosmos-Reason2-2B-W4A16-Edge2-FlashHead-clean}"
HF_TOKEN="${HF_TOKEN}"
...
echo "  FLASHHEAD_MODEL=${FLASHHEAD_MODEL}"
echo "  IMAGE=${IMAGE}"
...
if [ "${BENCHMARK_MODE}" = "python" ] || [ "${BENCHMARK_MODE}" = "both" ]; then
    ...
    echo "=== [Python] Baseline benchmark (no FlashHead plugin) ==="
    docker run ${DOCKER_COMMON} \
        ${IMAGE} \
        bash -c "python3 /opt/flash-head/tests/run_benchmark.py \
...
if [ "${BENCHMARK_MODE}" = "cli" ] || [ "${BENCHMARK_MODE}" = "both" ]; then
    ...
    echo "=== [CLI] Baseline benchmark (no FlashHead plugin) ==="
    docker run ${DOCKER_COMMON} \
        ${IMAGE} \
        bash -c "vllm bench latency \
            --model ${BASELINE_MODEL} \
...
docker run ${DOCKER_COMMON} \
    -e FLASHHEAD_MODEL=${FLASHHEAD_MODEL} \
